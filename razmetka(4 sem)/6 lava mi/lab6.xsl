...
<xsl:value-of select="name"/></td>
		   ...
		   <h3  style="margin:0"><xsl:value-of select ="sum(//energy)"/></h3>
		   <h3 style="margin:0">Average energy: </h3>
		   <h3  style="margin:0"><xsl:value-of select ="sum(//energy) div count(//energy)"/></h3>
			<h3 style="margin:0">Max energy: </h3>
			<xsl:variable name="max">
				 <xsl:for-each select="candies/chocolate-candy">
					<xsl:sort select="energy"/>
					<xsl:if test="position() = 1"><xsl:value-of select="energy"/></xsl:if>
				  </xsl:for-each>
			</xsl:variable>
		   ...
		   <h3  style="margin:0"><xsl:value-of select ="$min"/></h3>
